...
var _finders = _interopRequireDefault(require('./finders'));

var _flow_doctrine = _interopRequireDefault(require('../flow_doctrine'));
...
function inferParams(comment) {
  let path = _finders.default.findTarget(comment.context.ast);
...
function inferAndCombineParams(params, comment) {
  ...
  const mergedParamsAndErrors = mergeTrees(inferredParams, paramsToMerge); // Then merge the trees. This is the hard part.
  ...
  switch (param.type) {
    case 'AssignmentPattern': {
      ...
      return Object.assign(newAssignmentParam, {
        default: (0, _generator.default)(param.right, {
          compact: true
        }).code,
    ...
    case 'ObjectPattern': {
      ...
      if (prefix === '') {
        ...
        return {
          ...
          type: (param.typeAnnotation &&
            (0, _flow_doctrine.default)(param)) || {
            type: 'NameExpression',
            name: 'Object'
          },
          properties: _lodash.default.flatMap(param.properties, prop => {
      ...
      return _lodash.default.flatMap(param.properties, prop => {
    ...
    case 'ArrayPattern': {
      ...
      if (prefix === '') {
        return {
          ...
          // indices
          properties: _lodash.default.flatMap(
            param.elements,
            (element, idx) => {
      ...
      return _lodash.default.flatMap(param.elements, (element, idx) => {
        const indexedElement = Object.assign({}, element, {
          name: String(idx)
        });
    ...
    case 'RestElement': {
      ...
      if (param.typeAnnotation) {
        type.expression = (0, _flow_doctrine.default)(
    ...
    default: {
      ...
      if (param.typeAnnotation && param.typeAnnotation.typeAnnotation) {
        newParam.type = (0, _flow_doctrine.default)(
          param.typeAnnotation.typeAnnotation
        );
      }
...
function mergeNodes(inferred, explicit) {
  const intersection = _lodash.default.intersectionBy(
    inferred,
    explicit,
    tag => tag.name
  );

  const explicitOnly = _lodash.default.differenceBy(
    explicit,
    inferred,
    tag => tag.name
  );

  const inferredOnly = _lodash.default.differenceBy(
    inferred,
    explicit,
    tag => tag.name
  );
